...
class BranchAndBoundSolver(KnapsackSolver):
        ...
        while True:
            try:
                ...
                if self.is_feasible_assignment(assignment, assignment_depth):
                    ...
                    if (
                        assignment_depth <= self.running_max_assignment_depth and not 
                        BoundEstimate.is_estimate_within_epsilon_bound_of_or_gt_current_max_estimate(
                            estimate, self.running_max_estimate, self.epsilon)
                    ):
                        debug_print(
                            "discarding assignment with estimate {}, while running_max_assignment is {} with estimate {}"
                            .format(assignment, estimate, self.running_max_assignment, self.running_max_estimate), debug=self.debug)
                        continue
    ...
    def best_first_search(self):
        ...
        while not heap.is_empty():
            ...
            estimate = node.value
            debug_print(
                "bfs: current assignment is: {} of depth {}"
                .format(assignment, assignment_depth), debug=self.debug)
            ...
            if (
                ...
                if (
                    assignment_depth > self.running_max_assignment_depth or
                    (assignment_depth == self.running_max_assignment_depth and estimate > self.running_max_estimate)
                ):
                    self.running_max_estimate = estimate
                    self.running_max_assignment = assignment
                    self.running_max_assignment_depth = assignment_depth
                ...
                if assignment_depth < self.item_count:
                    for child in [[0], [1]]:
                        ...
                        debug_print(
                            "bfs: new assignment is: {}".format(new_assignment), debug=self.debug)
                        
                        if self.is_feasible_assignment(new_assignment, new_assignment_depth):
                            heap.add_to_heap(
                                HeapNode(new_assignment, new_assignment_depth,
                                         self.get_estimate(self.items, self.capacity, self.item_count, new_assignment)))
    ...
    def _lds_probe(self, assignment, assignment_depth, discrepancies):
        ...
        if (
            assignment_depth < self.running_max_assignment_depth and not
            BoundEstimate.is_estimate_within_epsilon_bound_of_or_gt_current_max_estimate(
                assignment_estimate, self.running_max_estimate, self.epsilon)
        ):
            ...
            assignment_depth > self.running_max_assignment_depth or
            (assignment_depth == self.running_max_assignment_depth and assignment_estimate >= self.running_max_estimate)
        ):
            debug_print(
    ...
    def solve(self):
        ...
        # use len(self.running_max_assignment) instead of self.item_count because in LDS we may not evaluate every assignment depth
        self.picked_items = [self.items[i].index for i in range(len(self.running_max_assignment)) if self.running_max_assignment[i] == 1]
